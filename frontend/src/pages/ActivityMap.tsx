...
import { Timeline, Card, message, Spin, Layout, Typography, Button } from 'antd';
...
const AMAP_CONFIG = {
  key: '21f3128468958dafae915d025ee8bbd2',
  securityJsCode: 'b43bae9c99b7c7e279248c86b4c889a2',
  version: '2.0'
};
      ...
      try {
        ...
        const simplifiedPersonData = {
          ...
          basic_info: personData.basic_info ? {
            ...
            name_chn: convertToSimplified(personData.basic_info.name_chn || '')
          } : undefined
        };
        setPerson(simplifiedPersonData);
        
        console.log('获取活动轨迹数据');
        const activitiesData = await api.getPersonActivities(parseInt(personId));
  ...
  useEffect(() => {
    if (!mapLoading && mapRef.current && activities.length > 0) {
      ...
      try {
        ...
        // 创建轨迹线
        console.log('创建轨迹线');
        const path = validActivities.map(activity => [activity.longitude, activity.latitude] as [number, number]);
        ...
        validActivities.forEach((activity, index) => {
          const marker = new window.AMap.Marker({
            position: [activity.longitude, activity.latitude] as [number, number],
    ...
    .map((activity, index) => ({
      ...
      children: (
        <Card size="small" style={{ width: 300 }}>
          <p><strong>地点：</strong>{activity.location_name}</p>
          <p><strong>类型：</strong>{activity.relation_type}</p>
          <p><strong>时间：</strong>{activity.first_year} - {activity.last_year}</p>
        </Card>
      ),
    }));
    ...
    <>
      ...
      <Layout style={{ minHeight: '100vh' }}>
        <Content style={{ padding: '24px' }}>
          ...
          <Card>
            <Title level={3}>{person?.basic_info?.name_chn || '未知'} 的活动轨迹</Title>
